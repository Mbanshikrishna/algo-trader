...
class AngelOneClient:
    ...
    _REFRESH_COOLDOWN = 5.0  # Seconds — skip re-login if one just succeeded.

    def refresh_session(self, *, force: bool = True) -> None:
        """Re-login and update the access token. Thread-safe.

        If force=False, skip if a refresh succeeded within the last
        _REFRESH_COOLDOWN seconds (prevents 403 flood when many requests
        hit auth failure simultaneously).
        """
        with self._login_lock:
            if not self._pin or not self._totp_secret:
                raise RuntimeError("Cannot refresh session: login credentials not stored")

            # Cooldown: if another thread just refreshed, reuse that token.
            if not force:
                since_last = time.monotonic() - self._last_login_time
                if since_last < self._REFRESH_COOLDOWN:
                    logger.info("[auth_refresh] Skipped — refreshed %.1fs ago.", since_last)
                    return
    ...
    def _request_with_retry(
        ...
        for attempt in range(_MAX_RETRIES):
            ...
            try:
                ...
                # Use force=False so concurrent 403s reuse a recent refresh
                # instead of hammering the login endpoint.
                response_text = response.text.lower()
                is_expired_session = response.status_code == 403 and any(
                    marker in response_text
                    for marker in ("invalid token", "token expired", "session expired", "jwt")
                )
                if response.status_code in _AUTH_FAIL_CODES or is_expired_session:
                    if attempt < _MAX_RETRIES - 1 and self._pin and self._totp_secret:
                        ...
                        try:
                            self.refresh_session(force=False)
                ...
                if response.status_code in _RETRYABLE_CODES:
                    if attempt < _MAX_RETRIES - 1:
                        ...
                        try:
                            backoff = max(float(retry_after), _BACKOFF_BASE * (2 ** attempt))
