...
class DhanClient:
    ...
    def login(
        ...
        **kwargs: Any,
        ...
        """Create a DhanClient with auto-login via TOTP if no token is provided.

        If DHAN_ACCESS_TOKEN is empty but DHAN_PIN and DHAN_TOTP_SECRET are set,
        ...
        if not cid:
            raise ValueError("DHAN_CLIENT_ID must be set")

        # Auto-login via TOTP if no token provided.
        if not token:
            pin = os.getenv("DHAN_PIN", "")
            totp_secret = os.getenv("DHAN_TOTP_SECRET", "")
            if not pin or not totp_secret:
                raise ValueError(
                    "DHAN_ACCESS_TOKEN is empty. Set DHAN_PIN and DHAN_TOTP_SECRET "
                    "for auto-login, or provide a valid access token."
                )
            token = cls._generate_token_via_totp(cid, pin, totp_secret)
    ...
    @staticmethod
    def _generate_token_via_totp(client_id: str, pin: str, totp_secret: str) -> str:
        """Generate a fresh Dhan access token using PIN + TOTP.

        Uses the official DhanHQ DhanLogin class for reliable token generation.
        """
        import pyotp
        ...
        otp = totp.now()
        logger.info("Generating Dhan access token via TOTP for client %s...", client_id)

        try:
            dhan_login = DhanLogin(client_id)
            result = dhan_login.generate_token(pin, otp)

            if isinstance(result, dict) and result.get("status") == "error":
                raise ConnectionError(f"Dhan TOTP login failed: {result.get('message', result)}")

            token = result.get("accessToken", "") if isinstance(result, dict) else ""
            if not token:
                raise ConnectionError(f"Dhan TOTP login returned no token: {result}")

            logger.info("Dhan access token generated successfully via TOTP.")
            return token
        except ConnectionError:
            raise
        except Exception as exc:
            raise ConnectionError(f"Dhan TOTP login failed: {exc}") from exc
    ...
    def refresh_if_stale(self, max_age_seconds: int = 7200) -> None:
        """Regenerate access token via TOTP if session is older than max_age_seconds."""
        elapsed = time.monotonic() - self._login_time
        ...
        totp_secret = os.getenv("DHAN_TOTP_SECRET", "")
        if not pin or not totp_secret:
            logger.debug("Dhan token refresh skipped — no PIN/TOTP configured.")
            return

        try:
            new_token = self._generate_token_via_totp(self.client_id, pin, totp_secret)
            self.access_token = new_token
            self._session.headers["access-token"] = new_token
            self._login_time = time.monotonic()
            logger.info("Dhan access token refreshed via TOTP.")
        except Exception as exc:
            logger.warning("Dhan token refresh failed: %s — continuing with current token.", exc)
        ...
        if isinstance(orders, list):
            ...
            for o in orders:
                ...
                converted.append({
                    ...
                    "price": o.get("price", 0),
                    "filledshares": o.get("filledQty", o.get("tradedQuantity", 0)),
                    "averageprice": o.get("averageTradedPrice", o.get("price", 0)),
                })
