...
# NOTE: Update after each NIFTY rebalance (March and September).
...
MAX_GAIN_PCT = 8.0   # Maximum intraday gain — stocks above 8% have 3x worse avg P&L.
...
class MarketCheckResult:
    ...
    def format_report(self) -> str:
        ...
        if self.bullish and self.score >= 3:
            lines.append(f"Decision: TRADE (score {self.score}/4 >= 3)")
        elif self.bullish and self.score == 2:
            lines.append(f"Decision: CONTRA-MOMENTUM MODE (score {self.score}/4)")
            lines.append("Rules: entry before 12:00 only, no re-entry")
        else:
            failed = []
...
def is_market_bullish(
...
) -> tuple[bool, MarketCheckResult]:
    """4-factor market bullishness check with 2-out-of-4 decision rule.
    ...
    def emit(reason: str = "") -> None:
        if event_sink is not None:
            event_sink(
                "market_gate",
                decision="trade" if result.bullish else "skip",
                reason=reason,
                exchange_timestamp=str(
                    next(
                        iter(market_data.get("fetched", [])), {}
                    ).get("exchFeedTime", "")
                ),
                payload={
                    "market_quotes": market_data.get("fetched", []),
                    "breadth_quotes": breadth_data.get("fetched", []),
    ...
    try:
        ...
        emit(f"market quote fetch failed: {exc}")
        return False, result
    ...
    if not nifty:
        ...
        emit("no Nifty 50 data returned")
        return False, result
    ...
    # If market data is unavailable (pre-market, holiday, weekend), skip.
    if result.nifty_ltp == 0 or result.nifty_open == 0:
        result.bullish = False
        emit("Nifty LTP or open unavailable")
        return False, result
    ...
    # Score >= 3: normal mode. Score == 2: contra-momentum mode.
    result.bullish = result.score >= 2

    emit("market score evaluated")
...
def _fetch_all_quotes(
    ...
    max_workers: int = 3,
    capture_errors: list[dict[str, Any]] | None = None,
) -> list[dict[str, Any]]:
    ...
    def _fetch_batch(batch: list[str]) -> list[dict[str, Any]]:
        try:
            ...
            if capture_errors is not None:
                capture_errors.append({"tokens": batch, "error": str(exc)})
            return []
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if capture_errors is not None:
                    capture_errors.append({"token": token, "error": str(exc)})
                result[token] = []

    return result


def score_candidate_quote(
    quote: dict[str, Any], completed_daily_candles: list[list],
) -> dict[str, Any] | None:
    """Score a point-in-time quote using only completed prior-day candles.

    This is the canonical production scoring function.  Keeping the historical
    backtest on this function prevents the ranking weights and basic filters
    from drifting away from live trading.
    """
    ...
    avg_prev_volume = 0.0
    if completed_daily_candles:
        prev_volumes = [float(c[5]) for c in completed_daily_candles if float(c[5]) > 0]
        if prev_volumes:
    ...
    prev_day_score = 0.0
    if len(completed_daily_candles) >= 2:
        closes = [float(c[4]) for c in completed_daily_candles]
        if len(closes) >= 2:
...
def _compute_score(quote: dict[str, Any], daily_candles: list[list]) -> dict[str, Any] | None:
    """Compatibility wrapper for broker data that normally includes today."""
    today = datetime.now(IST).date().isoformat()
    completed = [
        candle for candle in daily_candles
        if str(candle[0])[:10] < today
    ]
    return score_candidate_quote(quote, completed)
...
def scan_top_gainers(
    ...
    all_quotes = _fetch_all_quotes(client, stocks, capture_errors=quote_errors)
    ...
    if not candidates:
        if event_sink is not None:
            event_sink(
                ...
                reason="no quotes passed basic filters",
                payload={
    ...
    daily_data = _fetch_previous_day_candles(
        client, candidate_tokens, capture_errors=candle_errors
    )
    ...
    if event_sink is not None:
        event_sink(
            "scan_completed",
            decision="candidates_ranked" if scored else "no_candidates",
            reason="production composite score",
            payload={
                "universe_count": len(stocks),
                "quotes": all_quotes,
                "quote_errors": quote_errors,
                "daily_candles": daily_data,
                "daily_candle_errors": candle_errors,
